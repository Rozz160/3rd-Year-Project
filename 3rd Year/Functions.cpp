...
std::vector<coordinate*> normalvecs;       //all normal vectorsvectors
...
int objloader::load(const char* filename)
{
        ...
        for(int i=0;i<coord.size();i++) //and then go through all line and decide what kind of line it is
        {
                ...
                }else if((*coord[i])[0]=='v' && (*coord[i])[1]=='n')    //if it's a normal vector
                {
                        ...
                        normalvecs.push_back(new coordinate(tmpx,tmpy,tmpz));      //basically do the same
                        isnormals=true;
        ...
        if(materials.size()==0) //if some reason the material file doesn't contain any material, we don't have material
                ...
                ismaterial=true;
        std::cout << vertex.size() << " " << normalvecs.size() << " " << faces.size() << " " << materials.size() << std::endl;     //test purposes
        //draw
        int num;
                ...
                if(faces[i]->four)      //if quad
                {
                        glBegin(GL_QUADS);
                                if(isnormals)   //if there are normals
                                        glNormal3f(normalvecs[faces[i]->facenum-1]->x,normalvecs[faces[i]->facenum-1]->y,normalvecs[faces[i]->facenum-1]->z);    //use them
                        ...
                        glBegin(GL_TRIANGLES);
                                if(isnormals)   //if there are normals
                                        glNormal3f(normalvecs[faces[i]->facenum-1]->x,normalvecs[faces[i]->facenum-1]->y,normalvecs[faces[i]->facenum-1]->z);
...
unsigned int objloader::loadTexture(const char* filename)
{
        ...
        if(img2 == NULL)
        {
            std::cout << "Cannot convert image " << filename << std::endl;
            return -1;
        }
...
int objloader::showmenu(SDL_Surface* screen, TTF_Font* font)
{
    Uint32 time;
    int x, y;
    const int NUMMENU = 2;
    const char* labels[NUMMENU] = {"START","QUIT"};
